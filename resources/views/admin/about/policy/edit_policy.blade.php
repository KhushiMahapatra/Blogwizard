    ...
    <div class="container mt-4">
        ...
        <form action="{{ route('admin.policy.update') }}" method="POST">
            ...
            <div class="form-group">
                <textarea id="myeditpage" name="content" class="form-control" rows="10">{{ old('content', $policy->content) }}</textarea>
                @error('content')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <button type="submit" class="btn btn-primary mb-3">Update Policy</button>
        </form>
